...
class KelasModel extends Model
{
    ...
    public function getDataForDataTables($request)
    {
        ...
        $queryFiltered = "SELECT * from {$this->table}";
    ...
    public function insertData($data)
    {
        if ($this->driver == 'sqlsrv') {
            ...
            // eksekusi query untuk menyimpan ke database
            sqlsrv_query($this->db, "insert into {$this->table} (nama_kelas, nama_dpa, password_admin, tanggal_tugas) values(?,?,?,?)", array($data['nama_kelas'], $data['nama_dpa'], $data['password_admin'], $data['tanggal_tugas']));
        }
    }
...
?");
            ...
            // query untuk mengambil data berdasarkan id
            $query = sqlsrv_query($this->db, "select * from {$this->table} where id_kelas
= ?", [$id]);
            // ambil hasil query
    ...
    public function updateData($id, $data)
    {
        if ($this->driver == 'mysql') {
            ...
            // query untuk update data
            sqlsrv_query($this->db, "update {$this->table} set nama_kelas = ?, nama_dpa = ?, password_admin = ?, tanggal_tugas = ? where id_kelas = ?", [
                $data['nama_kelas'],
                $data['nama_dpa'],
                $data['password_admin'],
                $data['tanggal_tugas'],
